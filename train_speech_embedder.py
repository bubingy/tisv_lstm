...
def train(model_path):
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=hp.train.N, 
    ...
    for e in range(hp.train.epochs):
        ...
        for batch_id, mel_db_batch in enumerate(train_loader): 
            ...
            if (iteration + 1) % 10 == 0:
                mesg = "{0}\tEpoch: {1} Iteration:{2}\tLoss:{3:.4f}\tTLoss:{4:.4f}\t\n".format(time.ctime(),
                    e + 1, iteration, loss, total_loss / (batch_id + 1))
                print(mesg)
...
def test(model_path):
    ...
    test_loader = DataLoader(
        test_dataset, batch_size=hp.test.N, shuffle=True, 
